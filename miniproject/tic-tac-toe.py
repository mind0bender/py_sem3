...
def new_Btns():
    b1 = Button(root, width=20, height=10, command=lambda: onClick(1), text=mat[0][0])
...
def greet(exited):
    if exited:
        ...
        thnx = Label(
            text="Thanks for Trying out my WEIRD App",
            font="arial 20",
            width=30,
            height=10,
        )
        ...
        # just make a popup.
        messagebox.showinfo("Thanks", "Thanks for Trying out my WEIRD App")
...
# exit button.
exitbtn = Button(root, width=10, height=2, command=lambda: myexit(), text="EXIT")
...
def onClick(number):
    ...
    if (
        ...
        buttons[int((number - (((number - 1) % 3) + 1)) / 3)][
            int((number - 1) % 3)
        ].config(text=activePalyer, font="times 12")
        # updating the matrix.
        ...
        if someoneWon()["res"]:
            ...
            messagebox.showinfo(
                "Congratulations", someoneWon()["name"] + " won the match!"
            )
            reset()  # reseting the game.
        #  if the game is a tie.
        elif someoneWon()["tie"]:
            # popping up a dialog box.
            messagebox.showinfo("Tie", "This match was a TIE")
            reset()  # reseting the game.
